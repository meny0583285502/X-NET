...
$s.Add('')
$s.Add('# STEP 1: DNS -> 8.8.8.8 to reach GitHub')
$s.Add('Get-NetAdapter | Where-Object Status -eq Up | ForEach-Object {')
$s.Add('    try { Set-DnsClientServerAddress -InterfaceIndex $_.InterfaceIndex -ServerAddresses @("8.8.8.8") -EA SilentlyContinue } catch {}')
$s.Add('}')
$s.Add('ipconfig /flushdns | Out-Null')
$s.Add('Start-Sleep -Seconds 2')
$s.Add('')
$s.Add('# STEP 2: Fetch profile')
$s.Add('try {')
$s.Add('    $Praw = GH-Get "contents/profiles/$Safe.json"')
$s.Add('    $P = [System.Text.Encoding]::UTF8.GetString([Convert]::FromBase64String(($Praw.content -replace "\s",""))) | ConvertFrom-Json')
$s.Add('} catch {')
$s.Add('    Log "Profile FAIL: $_"')
...
$s.Add('    ipconfig /flushdns | Out-Null; exit')
$s.Add('}')
$s.Add('')
...
$s.Add('')
$s.Add('# STEP 8: Lock DNS -> 127.0.0.1')
$s.Add('Get-NetAdapter | Where-Object Status -eq Up | ForEach-Object {')
$s.Add('    try { Set-DnsClientServerAddress -InterfaceIndex $_.InterfaceIndex -ServerAddresses @("127.0.0.1") -EA SilentlyContinue } catch {}')
$s.Add('}')
