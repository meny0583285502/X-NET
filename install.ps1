...
$syncLines.Add('} catch {')
$syncLines.Add('    Log "Profile FAIL: $_"; exit')
$syncLines.Add('}')
$syncLines.Add('')
...
$syncLines.Add('Log "=== DONE: $ok domains ==="')
...
$trayLines.Add('        # sync.ps1 רץ כ-SYSTEM דרך Task Scheduler - הפעלה ישירה ממגש גורמת UAC
        schtasks /run /tn "XNET_Sync" 2>$null | Out-Null')
$trayLines.Add('    }')
$trayLines.Add('}')
$trayLines.Add('')
...
$trayLines.Add('$r1.add_Click({ $notify.Text = "X-NET - Syncing..."; Run-Sync; Start-Sleep 35; Update-Tray })')
...
if (-not $Silent) {
    ...
    Write-Host ""
    Write-Host "=== C:\XNET files ===" -ForegroundColor Cyan
    ...
    else { Write-Host "  (no log - sync may have failed)" -ForegroundColor Red }
