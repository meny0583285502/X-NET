...
$syncLines.Add('')
$syncLines.Add('# STEP 1: שחרר DNS זמנית כדי להגיע לאינטרנט')
...
$syncLines.Add('}')
$syncLines.Add('# הסר firewall blocks זמנית כדי שנוכל לפנות לגיטהאב')
...
$syncLines.Add('')
$syncLines.Add('# STEP 2: משוך פרופיל מגיטהאב')
$syncLines.Add('try {')
...
$syncLines.Add('    # אם נכשל - חזור לנעילה מיידית')
$syncLines.Add('    Get-NetAdapter | Where-Object Status -eq Up | ForEach-Object {')
...
$syncLines.Add('}')
$syncLines.Add('')
$syncLines.Add('# STEP 3: הסרה מלאה')
...
$syncLines.Add('}')
$syncLines.Add('')
$syncLines.Add('# STEP 5: בנה רשימת דומיינים מורשים')
...
$syncLines.Add('')
$syncLines.Add('# STEP 6: פתור IPs (DNS עדיין פתוח כאן!)')
...
$syncLines.Add('')
$syncLines.Add('# STEP 7: נעל DNS ל-127.0.0.1')
$syncLines.Add('Get-NetAdapter | Where-Object Status -eq Up | ForEach-Object {')
$syncLines.Add('    try { Set-DnsClientServerAddress -InterfaceIndex $_.InterfaceIndex -ServerAddresses @("127.0.0.1") -ErrorAction SilentlyContinue } catch {}')
$syncLines.Add('}')
...
$syncLines.Add('}')
$syncLines.Add('')
$syncLines.Add('# Block DoH בדפדפנים')
...
$syncLines.Add('')
$syncLines.Add('# STEP 8: כתוב hosts file')
$syncLines.Add('attrib -r $HOSTS 2>$null')
...
$syncLines.Add('')
$syncLines.Add('# STEP 9: Firewall - חסום הכל, פתח רק IPs מורשים')
$syncLines.Add('# מחק חוקים קיימים')
$syncLines.Add('Remove-NetFirewallRule -DisplayName "XNET-*" -ErrorAction SilentlyContinue')
$syncLines.Add('')
$syncLines.Add('# Telegram - תמיד חסום')
...
$syncLines.Add('')
$syncLines.Add('# חסום QUIC / VPN ports')
...
$syncLines.Add('')
$syncLines.Add('# Allow IPs מהרשימה הלבנה')
$syncLines.Add('if ($AllowedIPs.Count -gt 0) {')
$syncLines.Add('    $ipArray = @($AllowedIPs)')
$syncLines.Add('    # חלק ל-batches של 100 (מגבלת Windows Firewall)')
...
$syncLines.Add('}')
$syncLines.Add('')
$syncLines.Add('# Allow LAN (192.168.x.x, 10.x.x.x, 172.16-31.x.x)')
...
$syncLines.Add('')
$syncLines.Add('# חסום הכל שלא כוסה (חייב להיות אחרון!)')
$syncLines.Add('New-NetFirewallRule -DisplayName "XNET-BLOCK-ALL" -Direction Outbound -Action Block -Protocol Any -RemoteAddress Any -Profile Any -Priority 65000 -ErrorAction SilentlyContinue | Out-Null')
$syncLines.Add('')
$syncLines.Add('ipconfig /flushdns | Out-Null')
$syncLines.Add('')
$syncLines.Add('# STEP 10: שמור status.json')
...
$trayLines.Add('$notify.Text = "X-NET - טוען..."')
$trayLines.Add('')
...
$trayLines.Add('                $t = "X-NET - מושהה"')
$trayLines.Add('            } else {')
$trayLines.Add('                $notify.Icon = [System.Drawing.SystemIcons]::Shield')
$trayLines.Add('                $t = "X-NET | " + $s.allowed + " דומיינים | " + $s.ips + " IPs | " + $s.last_updated')
$trayLines.Add('            }')
$trayLines.Add('            if ($t.Length -gt 63) { $t = $t.Substring(0,60) + "..." }')
$trayLines.Add('            $notify.Text = $t')
$trayLines.Add('        } catch { $notify.Text = "X-NET פעיל" }')
$trayLines.Add('    }')
$trayLines.Add('}')
$trayLines.Add('')
$trayLines.Add('$menu = New-Object System.Windows.Forms.ContextMenu')
$trayLines.Add('$r1 = New-Object System.Windows.Forms.MenuItem "🔄 רענן עכשיו"')
$trayLines.Add('$r1.add_Click({ $notify.Text = "X-NET - מסנכרן..."; Run-Sync; Start-Sleep 40; Update-Tray })')
$trayLines.Add('$r2 = New-Object System.Windows.Forms.MenuItem "🌐 פתח לוח ניהול"')
...
$trayLines.Add('$r3 = New-Object System.Windows.Forms.MenuItem "📋 פתח לוג"')
$trayLines.Add('$r3.add_Click({ if (Test-Path "C:\XNET\sync_log.txt") { Start-Process notepad "C:\XNET\sync_log.txt" } })')
$trayLines.Add('$r4 = New-Object System.Windows.Forms.MenuItem "❌ סגור מגש"')
