...
if (-not ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]"Administrator")) {
    Start-Process powershell -ArgumentList "-ExecutionPolicy Bypass -File `"$PSCommandPath`" -UserEmail `"$UserEmail`"" -Verb RunAs; exit
}
...
$s.Add('# X-NET sync.ps1 v17.0')
$s.Add('$DIR    = "C:\XNET"')
$s.Add('$GH_IPS = @("140.82.112.5","140.82.113.5","140.82.114.5","140.82.121.5")')
$s.Add('$GH_RAW_IPS = @("185.199.108.133","185.199.109.133","185.199.110.133","185.199.111.133")')
...
$s.Add('function GH-API($path) {')
$s.Add('    $tok=(Get-Content "$DIR\gh_token.txt" -First 1 -EA SilentlyContinue).Trim()')
$s.Add('    foreach($ip in $GH_IPS) {')
$s.Add('        try { $h=@{Authorization="token $tok";"User-Agent"="XNET";Host="api.github.com"}; return Invoke-RestMethod "https://$ip/repos/meny0583285502/X-NET/$path" -Headers $h -UseBasicParsing -TimeoutSec 5 -EA Stop }')
$s.Add('        catch {}')
$s.Add('    }; return $null')
$s.Add('}')
$s.Add('function GH-RAW($path) {')
$s.Add('    foreach($ip in $GH_RAW_IPS) {')
$s.Add('        try { $h=@{"User-Agent"="XNET";Host="raw.githubusercontent.com"}; return Invoke-RestMethod "https://$ip/meny0583285502/X-NET/main/$path" -Headers $h -UseBasicParsing -TimeoutSec 5 -EA Stop }')
$s.Add('        catch {}')
$s.Add('    }; return $null')
$s.Add('}')
$s.Add('')
...
$s.Add('$Praw=GH-API "contents/profiles/$Safe.json"')
...
$s.Add('    Start-Process powershell -ArgumentList "-WindowStyle Hidden -ExecutionPolicy Bypass -File `"$env:TEMP\xnet_u.ps1`"" -Verb RunAs; exit')
$s.Add('}')
$s.Add('')
...
$s.Add('            netsh advfirewall set allprofiles firewallpolicy blockinbound,allowoutbound 2>$null | Out-Null')
...
$s.Add('    $Base=GH-RAW "base_whitelist.json"')
$s.Add('    if($Base) { $Base.allowed_domains|ForEach-Object{$d=$_.Trim() -replace "^https?://","" -replace "/$","" -replace "^www\.","";[void]$Allowed.Add($d)} }')
$s.Add('    else { Log "Base FAIL" }')
$s.Add('}')
...
$s.Add('')
$s.Add('# STEP 6: Lock DNS to 127.0.0.1')
...
$s.Add('ipconfig /flushdns | Out-Null')
$s.Add('')
$s.Add('# STEP 7: Firewall - block specific apps, allow all else')
$s.Add('Remove-NetFirewallRule -DisplayName "XNET-*" -EA SilentlyContinue')
$s.Add('# Block Telegram IPs')
$s.Add('New-NetFirewallRule -DisplayName "XNET-Block-Telegram" -Direction Outbound -Action Block -RemoteAddress @("149.154.160.0/20","91.108.4.0/22","91.108.8.0/22","91.108.56.0/22","95.161.64.0/20","91.105.192.0/23","185.76.151.0/24") -Protocol Any -Profile Any -EA SilentlyContinue | Out-Null')
$s.Add('# Block WhatsApp Desktop IPs')
$s.Add('New-NetFirewallRule -DisplayName "XNET-Block-WhatsApp" -Direction Outbound -Action Block -RemoteAddress @("157.240.0.0/16","31.13.64.0/18","185.60.216.0/22","179.60.192.0/22") -Protocol Any -Profile Any -EA SilentlyContinue | Out-Null')
$s.Add('# Block Windows News')
$s.Add('New-NetFirewallRule -DisplayName "XNET-Block-News" -Direction Outbound -Action Block -Program "%SystemRoot%\SystemApps\Microsoft.BingNews_8wekyb3d8bbwe\NewsAppPkg.exe" -Profile Any -EA SilentlyContinue | Out-Null')
$s.Add('# Block Firefox')
$s.Add('New-NetFirewallRule -DisplayName "XNET-Block-Firefox" -Direction Outbound -Action Block -Program "C:\Program Files\Mozilla Firefox\firefox.exe" -Profile Any -EA SilentlyContinue | Out-Null')
$s.Add('Log "Firewall rules applied"')
$s.Add('')
$s.Add('# STEP 8: Always ensure DNS server running and DNS locked')
...
$s.Add('    Start-Sleep 3; Log "DNS server started"')
$s.Add('}')
$s.Add('# Re-lock DNS after every sync (handles new network connections)')
$s.Add('Get-NetAdapter|Where-Object Status -eq Up|ForEach-Object{try{Set-DnsClientServerAddress -InterfaceIndex $_.InterfaceIndex -ServerAddresses @("127.0.0.1") -EA SilentlyContinue}catch{}}')
$s.Add('ipconfig /flushdns | Out-Null')
$s.Add('')
$s.Add('# STEP 9: Save status')
$s.Add('Log "=== DONE: $($AllowedList.Count) domains ==="')
$s.Add('@{installed=$true;paused=$false;base_sites_enabled=$P.base_sites_enabled;google_search_enabled=$P.google_search_enabled;last_updated=(Get-Date -Format "yyyy-MM-dd HH:mm:ss");user=$UserEmail;allowed=$($AllowedList.Count);version="17.0"}|ConvertTo-Json|Out-File "$DIR\status.json" -Encoding UTF8 -Force')
...
$w.Add('function Get-SHA($safe){$tok=(Get-Content "$DIR\gh_token.txt" -First 1 -EA SilentlyContinue).Trim();foreach($ip in $GH_IPS){try{$h=@{Authorization="token $tok";"User-Agent"="XNET";Host="api.github.com"};return(Invoke-RestMethod "https://$ip/repos/meny0583285502/X-NET/contents/profiles/$safe.json" -Headers $h -UseBasicParsing -TimeoutSec 3 -EA Stop).sha}catch{}};return $null}')
...
$w.Add('while($true){')
$w.Add('    Start-Sleep 5')
$w.Add('    # Check DNS every loop - re-lock if changed')
$w.Add('    $dnsOK=$true')
$w.Add('    Get-NetAdapter|Where-Object Status -eq Up|ForEach-Object{')
$w.Add('        $cur=(Get-DnsClientServerAddress -InterfaceIndex $_.InterfaceIndex -AddressFamily IPv4 -EA SilentlyContinue).ServerAddresses')
$w.Add('        if($cur -notcontains "127.0.0.1"){')
$w.Add('            Set-DnsClientServerAddress -InterfaceIndex $_.InterfaceIndex -ServerAddresses @("127.0.0.1") -EA SilentlyContinue')
$w.Add('            $dnsOK=$false')
$w.Add('        }')
$w.Add('    }')
$w.Add('    if(-not $dnsOK){ ipconfig /flushdns | Out-Null; Log "DNS re-locked" }')
...
$d.Add('# X-NET DNS+HTTP Server v17.0')
...
$u.Add('Start-Sleep 1; Remove-Item "C:\XNET" -Recurse -Force -EA SilentlyContinue')
