...
$syncLines.Add('# STEP 1: Set explicit DNS so we can reach the internet instantly')
$syncLines.Add('Get-NetAdapter | Where-Object Status -eq Up | ForEach-Object {')
$syncLines.Add('    Set-DnsClientServerAddress -InterfaceIndex $_.InterfaceIndex -ServerAddresses ("8.8.8.8","8.8.4.4") -ErrorAction SilentlyContinue')
$syncLines.Add('}')
$syncLines.Add('ipconfig /flushdns | Out-Null')
$syncLines.Add('Start-Sleep -Seconds 3')
$syncLines.Add('')
$syncLines.Add('# STEP 2: Fetch profile from GitHub (with Cache-Busting)')
$syncLines.Add('try {')
$syncLines.Add('    $t = [DateTimeOffset]::UtcNow.ToUnixTimeSeconds()')
$syncLines.Add('    $P = Invoke-RestMethod "$GH_RAW/profiles/$Safe.json?t=$t" -UseBasicParsing -Headers @{"Cache-Control"="no-cache"} -ErrorAction Stop')
$syncLines.Add('} catch {')
